...
export const useCompanyStore = defineStore("companyStore", {
  ...
  actions: {
    ...
    async getCompany(companyId) {
      ...
      const res = await fetch(`https://bizethio-backend-production-daf6.up.railway.app/api/companies/${companyId}`, {
        method: "Get",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
    ...
    async updateCompany(payload, company) {
      ...
      const res = await fetch(`https://bizethio-backend-production-daf6.up.railway.app/api/companies/${company}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
