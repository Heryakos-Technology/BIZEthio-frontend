<template>
  <AdminLayout>
    <div class="pb-20">
      <div class="pb-10 px-5 w-1/2 mx-auto" v-if="hasApprovedOrRejected">
        <h1 class="text-center py-8 font-bold text-xl text-blue-700">
          Company Statistics
        </h1>
  ...
  <div class="flex bg-white py-3 mt-3 text-xs rounded-md px-2">
    <div class="break-words max-w-[80px] w-1/7">{{ campany.name }}</div>
        ...
        <h1 class="-mt-10 py-8 font-bold text-2xl text-blue-700">
          Featured Campaigns
        </h1>
        <div class="px-5">
        ...
        <div
          v-if="loadingCampanies && !hasApprovedOrRejected"
        ...
        <div class="flex">
          ...
          <div>
            ...
            <button
              ...
              class="mt-20 bg-red-500 rounded-full w-10 h-10 text-white p-2 ml-5"
            >
      ...
      <div
        v-if="noCampanyData"
        class="text-2xl text-center text-green-700 font-bold"
      >
        {{ campanyData }}
      </div>
      <div v-for="campany in filteredCampanies" :key="campany.id" class="px-3">
      ...
      <div class="flex justify-between w-1/2 mx-auto mt-5">
        ...
        <button
          class="number bg-white px-2 shadow-sm"
        ...
        <div>
          <button
            class="bg-white px-2 shadow-sm"
            :disabled="activeNumber === totalPages"
            @click="goToNext"
          >
            Next
          </button>
  ...
  setup() {
    ...
    const hasApprovedOrRejected = computed(() => {
      return (
        ...
        (chartData.value.datasets[0].data[0] > 0 ||
          chartData.value.datasets[0].data[1] > 0)
      );
    });
    ...
    const approveCampany = async () => {
      ...
      try {
        ...
        $toast.error("Error approving company try again!.", {
          position: "top",
        });
        singleampany.value = false;
      } finally {
    ...
    const rejectCampany = async () => {
      ...
      try {
        ...
        $toast.error("Error rejecting company try again!.", {
          position: "top",
        });
        singleampany.value = false;
        showRejectConfirm.value = false;
