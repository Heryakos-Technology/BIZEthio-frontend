...
export default {
  ...
  handleFileDrop(event) {
    ...
    updatePasswordStrength() {
      const { label, class: strengthClass } = this.checkPasswordStrength(
    ...
    validateFields() {
      ...
      if (this.showPassword) {
        ...
        if (
          ...
          this.companies.password !== this.companies.password_confirmation
        )
          ...
          if (currentUser.emailVerified) {
            ...
            if (success) {
              const response = await axios.post(
                `https://bizethio-backend-production-daf6.up.railway.app/api/company/register`,
                companyData,
                {
                  headers: {
    ...
    async fetchCategories() {
      try {
        const response = await axios.get(
          "https://bizethio-backend-production-daf6.up.railway.app/api/categories"
        );
