...
onMounted(async () => {
  try {
    ...
    companies.value = allCompanies.filter((company) => {
      return company.category && company.category.name === route.params.name;
    });
...
<template>
  <UserLayout>
    <div class="px-4 xl:max-w-[1200px] mx-auto sm:mt-16 lg:pb-16">
      <button
        @click="router.back()"
        class="mb-4 text-primaryColor cursor-pointer hover:text-lightBlue group flex  gap-x-2 items-center"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="fill-primaryColor size-10 group-hover:fill-lightBlue"
          viewBox="0 0 256 256"
        >
          <path
      ...
      >
        Companies in {{ route.params.name }}
      </h1>
      ...
      <div v-else-if="companies.length === 0" class="py-20 text-center">
        ...
        <h3 class="mt-2 text-lg font-medium text-gray-900">
          No companies found with this category
        </h3>
